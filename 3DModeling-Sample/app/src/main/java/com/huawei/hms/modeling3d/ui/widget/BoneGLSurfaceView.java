...
import com.huawei.hms.modeling3d.utils.ShaderUtils;
...
public class BoneGLSurfaceView implements GLSurfaceView.Renderer {
    ...
    public BoneGLSurfaceView() {
        vertexBuffer = ByteBuffer.allocateDirect(bonePositions.length * 4).order(ByteOrder.nativeOrder()).asFloatBuffer();
    }

    @Override
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        GLES30.glClearColor(0.8f, 0.8f, 0.8f, 1.0f);
        shaderProgram = ShaderUtils.createProgram(vertextShader, fragmentShader);
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        ...
        modelViewProjectionBuffer = ByteBuffer.allocateDirect(modelViewProjection.length * 4).order(ByteOrder.nativeOrder()).asFloatBuffer();
    ...
    private void updateVertexBuffer() {
        ...
        for (int i = 0; i < bonePairs.length; i++) {
            bonePositions[index++] = lerpedPositions[bonePairs[i][0] * 3];
            bonePositions[index++] = lerpedPositions[bonePairs[i][0] * 3 + 1];
            bonePositions[index++] = lerpedPositions[bonePairs[i][0] * 3 + 2];
            bonePositions[index++] = lerpedPositions[bonePairs[i][1] * 3];
            bonePositions[index++] = lerpedPositions[bonePairs[i][1] * 3 + 1];
            bonePositions[index++] = lerpedPositions[bonePairs[i][1] * 3 + 2];
        }
