...
import com.huawei.hms.modeling3d.ui.activity.ArSceneViewActivity;
import com.huawei.hms.modeling3d.ui.activity.ScanAgainActivity;
...
public class HandlerPopDialog {
    ...
    TextView tvDownload;
    TextView tvArPreview;
    TextView tvScanSgain;
    ...
    private void initView(View contentView) {
        ...
        tvArPreview = contentView.findViewById(R.id.tv_show_ar);
        tvScanSgain = contentView.findViewById(R.id.tv_scan_again);
        if (appDb.getStatus() == ConstantBean.MODELS_RECONSTRUCT_COMPLETED_STATUS) {
            tvDownload.setVisibility(View.VISIBLE);
            tvArPreview.setVisibility(View.VISIBLE);
            tvScanSgain.setVisibility(View.GONE);
            ...
            if (status==Modeling3dReconstructConstants.RestrictStatus.UNRESTRICT){
                ...
                ((TextView)contentView.findViewById(R.id.tv_delete)).setVisibility(View.VISIBLE);
                ...
                ((TextView)contentView.findViewById(R.id.tv_delete)).setVisibility(View.INVISIBLE);
            }
        } else {
            tvDownload.setVisibility(View.GONE);
            tvArPreview.setVisibility(View.GONE);
            tvScanSgain.setVisibility(View.GONE);
        }
        ...
        contentView.findViewById(R.id.tv_delete).setOnClickListener(v -> {
            if (status==Modeling3dReconstructConstants.RestrictStatus.UNRESTRICT) {
                ...
                Toast.makeText(mContext,mContext.getText(R.string.restricted_text),Toast.LENGTH_SHORT).show();
            }
        });
        ...
        tvArPreview.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                String savePath = TaskInfoAppDbUtils.getTasksByTaskId(appDb.getTaskId()).getFileSavePath();
                if (!TextUtils.isEmpty(savePath)){
                    File file = new File(savePath);
                    if (Objects.requireNonNull(file.listFiles()).length == 2){
                        for (File fileName : file.listFiles()){
                            if (fileName.getAbsolutePath().contains("gltf")){
                                Intent intent = new Intent(mContext, ArSceneViewActivity.class);
                                intent.putExtra("path",savePath);
                                mContext.startActivity(intent);
                            }
                        }
                    }else {
                        ToastUtil.showToast(mContext,"Download the GLTF file, only some Huawei phones with Arengine are supported");
                    }
                }
                if (popupWindow != null) {
                    popupWindow.dismiss();
        ...
        tvScanSgain.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(mContext, ScanAgainActivity.class);
                intent.putExtra("taskId",appDb.getTaskId());
                mContext.startActivity(intent);
            }
        });
    }
